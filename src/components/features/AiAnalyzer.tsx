    ...
    <div className="space-y-5 animate-in zoom-in-95 duration-500 pb-20">
      ...
      <div className="space-y-2">
        {Object.entries(question.options).map(([key, value]: [string, string]) => (
          <button
            ...
            className={`w-full p-4 text-left rounded-[1.5rem] border-2 transition-all text-[13px] flex items-center gap-3 ${selectedOption === key
                ? 'border-rose-500 bg-rose-50 text-rose-700 font-bold shadow-md'
                : 'border-slate-100 bg-white hover:border-slate-200'
              }`}
          >
            <span className={`w-8 h-8 rounded-xl flex items-center justify-center text-[10px] font-black ${selectedOption === key ? 'bg-rose-500 text-white' : 'bg-slate-100 text-slate-400'
              }`}>{key}</span>
      ...
      {showFeedback && (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
          <div className={`p-6 rounded-[2.5rem] text-[13px] border-2 shadow-inner ${selectedOption === question.correct ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'
            }`}>
